...
public class ReflectedXSSTable extends JTable implements ActionListener{
    ...
    TableRowSorter<ReflectedXSSTableModel> sorter;

    public ReflectedXSSTable(ReflectedXSSTableModel tableModel, ReflectedXSSMain controller) {
        super(tableModel);
...
//        stdout = new PrintWriter(controller.callbacks.getStdout(), true);
        ...
        getColumnModel().getColumn(8).setMaxWidth(50);
    ...
    public void changeSelection(int rowIndex, int columnIndex, boolean toggle, boolean extend) {
        ...
        ReflectedXSSEntry refXssRow = controller.httpHistoryModel.reflectedXssList.get(rowIndex);
        // Parameter Table 내용 업데이트
        controller.paramTableModel.setData(refXssRow.doubted_params);
        controller.requestViewer.setMessage(refXssRow.httpRequestResponse.getRequest(), true);
        controller.responseViewer.setMessage(refXssRow.httpRequestResponse.getResponse(), refXssRow.doubted_params);
        controller.currentlyDisplayedItem = refXssRow.httpRequestResponse;
    }
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        List<ReflectedXSSEntry> historyList = controller.httpHistoryModel.reflectedXssList;
        int row = -1;
        ...
        if (row == -1)  // 이런 경우는 없을 거 같지만 일단...시큐어코딩은 중요하니깐...
            return;
        ReflectedXSSEntry targetRow = controller.httpHistoryModel.reflectedXssList.get(row);
        boolean useHttps = targetRow.url.getProtocol().equalsIgnoreCase("https");
        ...
        if (menu == menuItemIntruder)
        {
            ...
            controller.callbacks.sendToIntruder(targetRow.url.getHost(), targetRow.url.getPort(), useHttps,
                    targetRow.httpRequestResponse.getRequest(), targetRow.httpRequestResponse.getRequestMarkers());
        }
        else if (menu == menuItemRepeater)
        {
            // Send the request to the Repeater
            controller.callbacks.sendToRepeater(targetRow.url.getHost(), targetRow.url.getPort(), useHttps, targetRow.httpRequestResponse.getRequest(),null);
        }
        else if (menu == menuItemCopyURL)
        {
            // Copy URL to the clipboard
            StringSelection stringSelection = new StringSelection (targetRow.url.toString());
            Clipboard clpbrd = Toolkit.getDefaultToolkit ().getSystemClipboard ();
            clpbrd.setContents (stringSelection, null);
        }
        else if (menu == menuItemDeleteItem)
        {
            controller.httpHistoryModel.removeRow(row);
            controller.paramTableModel.dataList.remove(row);

            // Clear request/response
            controller.requestViewer.setMessage(new byte[0], true);
            controller.responseViewer.setMessage(new byte[0], new ArrayList<>());

            //Reload the request table
            controller.paramTableModel.fireTableDataChanged();
        }
        else if (menu == menuItemClearList)
        {
            controller.paramTableModel.dataList.clear();
            controller.httpHistoryModel.reflectedXssList.clear();
            ...
            //Reload the request table
            controller.paramTableModel.fireTableDataChanged();
            controller.httpHistoryModel.fireTableDataChanged();
        }
    }
}
